//
//  CombineListSectionController.swift
//  AktaFeedCreator
//
...
import IGListKit
import RxSwift
class CombineListSectionController: ListSectionController {
    private var collection: FeedCollection!
    private var backgroundColor : UIColor = .white
    
    var feedSubject : BehaviorSubject<FeedCollection> =  BehaviorSubject(value: FeedCollection())
    ...
    override func didSelectItem(at index: Int) {
        self.backgroundColor = aktaGreen.withAlphaComponent(0.1)
        self.collectionContext?.performBatch(animated: true, updates: { (batchContext) in
            batchContext.reload(self)
        })
        self.feedSubject.onNext(self.collection)
    }
}
